...
public class DecodeUnsigned8Test {

    @Test
    public void testDecodeUnsigned8() throws UtilityException, DataTypeLengthMismatchException, UnknownHostException, MissingAbstractDataTypeException {
        byte[] data = IpfixUtilities.hexStringToByteArray("02");
        int length = 1;
        IpfixDataRecordDecodeMethod method = new DecodeUnsigned8();
        String value = method.decodeDataRecord(data, length);
        assertEquals("2", value);
    }

    @Test(expected = DataTypeLengthMismatchException.class)
    public void testDecodeUnsigned8LengthMismatch() throws UtilityException, DataTypeLengthMismatchException, UnknownHostException, MissingAbstractDataTypeException {
        byte[] data = IpfixUtilities.hexStringToByteArray("02");
        int length = 2;
        IpfixDataRecordDecodeMethod method = new DecodeUnsigned8();
        String value = method.decodeDataRecord(data, length);
