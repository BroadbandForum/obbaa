...
public class DecodeSigned16 implements IpfixDataRecordDecodeMethod {
    ...
    public String decodeDataRecord(byte[] data, int length) throws UtilityException, DataTypeLengthMismatchException {
        if (length != LENGTH) {
            throw new DataTypeLengthMismatchException("Incorrect length for Signed16 type: " + length);
        }
        data = IpfixUtilities.copyByteArray(data, 0, LENGTH);
        return Integer.toString((IpfixUtilities.byteToInteger(data, 0, LENGTH)));
    }
