...
import static org.broadband_forum.obbaa.ipfix.collector.util.IpfixConstants.buildCacheKey;
...
public class IpfixCachingServiceImpl implements IpfixCachingService {
    ...
    public void cacheTemplateRecord(long obsvDomain, String hostname, Map<Integer, AbstractTemplateRecord> templateByIdCache) {
        ...
        String prefix = buildCacheKey(obsvDomain, hostname);
        templateByIdCache.forEach((templateId, templateRecord) -> {
            m_ipfixTemplateRecordCache.put(new Element(buildCacheKey(prefix, templateId), templateRecord));
        });
    }

    @Override
    public AbstractTemplateRecord getTemplateRecord(long obsvDomain, String hostname, int templateId) {
        return (AbstractTemplateRecord) m_ipfixTemplateRecordCache.get(buildCacheKey(obsvDomain, hostname, templateId)).getObjectValue();
    }
    ...
    public void updateHostName(long obsvDomain, String oldHostName, String newHostName) {
        String oldPrefix = buildCacheKey(obsvDomain, oldHostName);
        String newPrefix = buildCacheKey(obsvDomain, newHostName);
        ...
        keys.stream().filter(key -> key.startsWith(oldPrefix))
                .forEach(key -> {
                    ...
                    m_ipfixDataSetCache.put(new Element(buildCacheKey(newPrefix, set.getHeader().getId()), set));
                });
        ...
        keys.stream().filter(key -> key.startsWith(oldPrefix))
                .forEach(key -> {
                    ...
                    m_ipfixTemplateRecordCache.put(new Element(buildCacheKey(newPrefix, IpfixUtilities.getTemplateId(template)), template));
                });
    }

    @Override
    public void cacheIpfixDataSet(long obsvDomain, String hostName, IpfixDataSet set) {
        m_ipfixDataSetCache.put(new Element(buildCacheKey(obsvDomain, hostName, set.getHeader().getId()), set));
    }
