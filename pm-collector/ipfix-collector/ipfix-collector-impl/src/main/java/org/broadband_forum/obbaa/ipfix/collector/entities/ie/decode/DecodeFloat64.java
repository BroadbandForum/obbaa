...
public class DecodeFloat64 implements IpfixDataRecordDecodeMethod {
    ...
    public String decodeDataRecord(byte[] data, int length) throws UtilityException, DataTypeLengthMismatchException {
        if (length != LENGTH) {
            throw new DataTypeLengthMismatchException("Incorrect length for Float64 type: " + length);
        }

        byte[] stringOctets = IpfixUtilities.copyByteArray(data, 0, length);
        return Float.toString((IpfixUtilities.bytesToFloat(data, LENGTH)));
    }
}
