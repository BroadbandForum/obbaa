...
public class DecodeEnumeration implements IpfixDataRecordDecodeMethod {
    ...
    public String decodeDataRecord(byte[] data, int length) throws UtilityException, DataTypeLengthMismatchException {
        if (length != LENGTH) {
            throw new DataTypeLengthMismatchException("Incorrect length for Enumeration type: " + length);
        }

        data = IpfixUtilities.copyByteArray(data, 0, LENGTH);
        return IpfixUtilities.bytesToEnum(data, LENGTH);
    }
