...
RUN mkdir /ipfix
ARG version=5.0.0
ADD target/ipfix-collector-dist*.tar.gz /ipfix

RUN ln -s /ipfix/ipfix-collector-dist-${version} /ipfix/ipfix-collector-dist

ENV ver $version
CMD /ipfix/ipfix-collector-dist/bin/start.sh
