...
public class DecodeUnion implements IpfixDataRecordDecodeMethod {

    private static final int MIN_LENGTH = 2;

    @Override
    public String decodeDataRecord(byte[] data, int length) throws UtilityException, DataTypeLengthMismatchException,
            MissingAbstractDataTypeException, UnknownHostException {
        if (length < MIN_LENGTH) {
            throw new DataTypeLengthMismatchException("Incorrect length for Union type: " + length);
        }

        return IpfixUtilities.bytesToUnion(data);
    }
